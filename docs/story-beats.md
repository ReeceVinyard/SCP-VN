...
- Leave without ID → `missing_researcher_id` (SCPs assign name later).
...
| **Middle** | center | SCP appears on second line while reading |
| Right | east | Flavor roster |

SCP encounter: calm / flee / offer notes (if left or right paper read).
...
| `named_by_scp` | (future) SCP naming scene |
